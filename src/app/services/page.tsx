...
export default function ServicesPage() {
  ...
  const atLimit = isAtLimit('services');
  const nearLimit = isNearLimit('services');
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-0.5">
            {total} services available{usageLabel('services') ? ` · ${usageLabel('services')} active` : ''}
          </p>
        </div>
        <button
          ...
          disabled={atLimit}
          title={atLimit ? 'Service limit reached. Visit Subscription to increase.' : undefined}
        >
      ...
      {atLimit && (
        <div className="flex items-start gap-2 text-red-700 bg-red-50 border border-red-200 p-3 rounded-lg text-sm">
          ...
          <span>Active service limit reached ({usageLabel('services')}). <a href="/subscription" className="underline font-medium">Visit Subscription</a> to increase your limit.</span>
        </div>
      )}
      {!atLimit && nearLimit && (
        <div className="flex items-start gap-2 text-orange-700 bg-orange-50 border border-orange-200 p-3 rounded-lg text-sm">
          <AlertCircle className="w-4 h-4 mt-0.5 shrink-0" />
          <span>Approaching active service limit ({usageLabel('services')}). <a href="/subscription" className="underline font-medium">Visit Subscription</a> to increase your limit.</span>
        </div>
      )}
